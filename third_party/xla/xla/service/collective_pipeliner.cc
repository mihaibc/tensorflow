...
class WhileLoopAnalysis {
 ...
 private:
  ...
  // Precomputed TuplePointsTo analysis for the HLO module containing `while_`.
...
absl::StatusOr<bool> CollectivePipeliner::RunPipeliner(
  ...
  TF_ASSIGN_OR_RETURN(std::unique_ptr<TuplePointsToAnalysis> tuple_to_points,
                      TuplePointsToAnalysis::Run(module));
  std::unique_ptr<CallGraph> call_graph = CallGraph::Build(module);

  std::vector<std::pair<HloInstruction*, WhileLoopAnalysis>> loop_analyses;
  for (HloComputation* computation : module->MakeComputationPostOrder()) {
    for (HloInstruction* instruction :
      ...
      loop_analyses.push_back(std::make_pair(
          instruction,
          WhileLoopAnalysis(instruction, config_.max_pipelining_per_loop,
                            config_.pipeline_use_tree,
                            config_.process_different_sized_ops,
                            tuple_to_points.get(), call_graph.get())));
      WhileLoopAnalysis& loop_analysis = loop_analyses.back().second;
